...
class DaoRepresentante
{
    ...
    function registroRepresentante(Representante $re)
    {
...
<body class="clean-body" style="margin: 0; padding: 0; -webkit-text-size-adjust: 100%; background-color: #091548;">
    ...
    function modificarRepresentante(Representante $re)
    {
        if (!($re instanceof Representante)) {
            $this->Error = "Error de instanciado,\n el objeto no es de tipo Clase Representante";
            return 0;
    ...
    function dardebajaRepresentante($dui)
    {
    ...
    function BuscarRepresentante($dui)
    {
		$representante = null;
        $consulta = $this->Conexion_ID->query("SELECT dui as 'dui', nombre as 'nombre', apellido as 'apellido', estado as 'estado', 'usuario' as 'tipo' FROM `representante` WHERE dui = '$dui'");
        
		if ($consulta) {
            if (is_object($consulta)) {
                $representante = $consulta->fetch_object();
